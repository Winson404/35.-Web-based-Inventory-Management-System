<title>IMS | Mechanic records</title>
        ...
        <div class="row">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Mechanic records</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
            ...
            <form action="../includes/processes.php" method="POST" enctype="multipart/form-data">
              ...
              <div class="card">
                ...
                <div class="card-footer">
                    <button type="submit" class="btn bg-primary float-right" name="update_mechanic" id="submit_button"><i class="fa-solid fa-floppy-disk"></i> Submit</button>
                    <a href="mechanic.php" class="btn bg-dark float-right mr-2"><i class="fas fa-arrow-left"></i> Back</a>
                </div>
              </div>
            </form>
            <?php } else { ?>
            <form action="../includes/processes.php" method="POST" enctype="multipart/form-data">
              <input type="hidden" class="form-control" name="mechanic_branch" value="<?= $assigned_branch ?>" readonly>
              <div class="card">
                <div class="card-body">
                    <div class="form-group">
                      <a class="h5 text-primary"><b>Basic information</b></a>
                ...
                <div class="card-footer">
                    <button type="submit" class="btn bg-primary float-right" name="save_mechanic" id="submit_button"><i class="fa-solid fa-floppy-disk"></i> Submit</button>
                </div>
              </div>
            </form>
            <?php } ?>
                  ...
                  <tbody id="users_data">
                      <?php 
                        ...
                        if($assigned_branch == 0) {
                          $sql = mysqli_query($conn, "SELECT * FROM mechanic ");
                        } else {
                          $sql = mysqli_query($conn, "SELECT * FROM mechanic WHERE mechanic_branch=$assigned_branch ");
                        }
                    ...
                    <tr>
                        ...
                        <td>
                          <a class="btn btn-info btn-xs" href="mechanic.php?Id=<?php echo $row['Id']; ?>" <?php if($u_type == 'Staff') { echo 'style="pointer-events: none; opacity: .5"'; } ?>><i class="fas fa-pencil-alt"></i> Edit</a>
                          <button type="button" class="btn bg-danger btn-xs" data-toggle="modal" data-target="#delete<?php echo $row['Id']; ?>" <?php if($u_type == 'Staff') { echo 'disabled'; } ?>><i class="fas fa-trash"></i> Delete</button>
                          <button type="button" class="btn bg-warning btn-xs" data-toggle="modal" data-target="#status<?php echo $row['Id']; ?>" <?php if($u_type == 'Staff') { echo 'disabled'; } ?>><i class="fas fa-pencil-alt"></i> Status</button>
                        </td> 
                    </tr>

                    <?php include 'mechanic_delete.php'; } ?>
